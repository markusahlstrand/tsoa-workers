...
async function getAuth0Jwks(ctx: Context<Env>) {
  const jwksUri = ctx.env.AUTH0_JWKS_URL;

  if (!jwksUri.length) {
    throw new Error('JKS url missing');
  }
...
function isValidScopes(token: TokenData, scopes: string[]) {
  ...
  console.log('TokenScopes: ' + JSON.stringify(tokenScopes));
...
async function isValidJwtSignature(ctx: Context<Env>, token: TokenData) {
  ...
  const jwkKeys = await getAuth0Jwks(ctx);
